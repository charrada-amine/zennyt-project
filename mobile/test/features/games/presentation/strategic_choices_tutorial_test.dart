...
import 'package:zennyt/features/games/presentation/widgets/strategic_choices_tutorial.dart';
...
void main() {
          ...
          body: SafeArea(
            child: MediaQuery(
              ...
              child: RepaintBoundary(
                key: const ValueKey('strategic-tutorial-capture'),
  ...
  testWidgets(
    ...
    (tester) async {
      for (final path in kStrategicChoicesTutorialAssets) {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final context = tester.element(find.byType(StrategicChoicesTutorial));
      for (var page = 0; page < 5; page++) {
        await tester.runAsync(() async {
          final provider = tester
              .widget<Image>(
                find.byKey(ValueKey('strategic-tutorial-image-$page')),
              )
              .image;
        ...
        expect(
          tester
              .widget<RawImage>(
                find.descendant(
                  of: find.byKey(ValueKey('strategic-tutorial-image-$page')),
        ...
        await expectLater(
          find.byKey(const ValueKey('strategic-tutorial-capture')),
          matchesGoldenFile(
            'goldens/strategic-choices-tutorial-${page + 1}.png',
          ),
        );
        if (page < 4) {
          await tester.tap(find.text('Suivant'));
