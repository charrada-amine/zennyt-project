...
import 'package:zennyt/features/games/presentation/widgets/emotional_radar_tutorial.dart';
...
void main() {
          ...
          body: SafeArea(
            child: MediaQuery(
              ...
              child: RepaintBoundary(
                key: const ValueKey('radar-tutorial-capture'),
  ...
  testWidgets(
    ...
    (tester) async {
      for (final path in kEmotionalRadarTutorialAssets) {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final context = tester.element(find.byType(EmotionalRadarTutorial));
      await tester.runAsync(() async {
        for (final path in kEmotionalRadarTutorialAssets) {
          await precacheImage(
            ResizeImage(AssetImage(path), width: 390),
            context,
          );
        }
      });
      for (var page = 0; page < 5; page++) {
        await tester.runAsync(() async {
          final provider = tester
              .widget<Image>(find.byKey(ValueKey('radar-tutorial-image-$page')))
              .image;
        ...
        expect(
          tester
              .widget<RawImage>(
                find.descendant(
                  of: find.byKey(ValueKey('radar-tutorial-image-$page')),
        ...
        await expectLater(
          find.byKey(const ValueKey('radar-tutorial-capture')),
          matchesGoldenFile('goldens/emotional-radar-tutorial-${page + 1}.png'),
        );
        if (page < 4) {
          await tester.tap(find.text('Suivant'));
          await tester.pumpAndSettle();
