...
> Since Flyway was removed, the schema is generated by Hibernate from the JPA entities at
> startup (plus `db/schema-complements.sql` and `db/reference-data.sql`); leftover
> `target/classes/db/migration` copies from older branches are ignored.
