...
| Memory Quest — « J'investigue » | `MEMORY_QUEST` | Mémoire de travail | 🟢 **A + B + distraction jouables + backend** (composite serveur /100, parité mock) ; reste : niveaux | Écran custom Flutter |
...
| | `domain/config/MemoryQuestConfig.java` | **Barème « J'investigue »** : `taskScore(acc)` (0–5), bandes d'interprétation (provisoires), timings d'encodage. Java pur. |
...
| « J'investigue » — niveaux (4→12 objets, distraction gatée niveau ≥3) | 🔴 À venir (niveau 1 figé) |
| `DECISION` | 🔴 Déclaré au contrat, non implémenté |
