...
|-----|-------------------|-------|--------|-------|
| **Planifik — « Je planifie »** | `PLANIFIK` | Planification | 🟢 **Complet /30** — 3 mini-jeux jouables (Chemin Optimal + Ordonnancement + Tour de Hanoï) | Flame + écrans Flutter custom |
| **Move Fast — « Je bouge »** | `MOVE_FAST` | Flexibilité cognitive | 🟢 **Complet** — jouable | Écran Flutter custom |
| **Memory Quest — « J'investigue »** | `MEMORY_QUEST` | Mémoire de travail | 🟢 **Complet** : A + B + distraction, système de 7 niveaux (3→9), calibrage → timeout (1er module dont le score dépend du temps), `session_valid` ; composite serveur /100, parité mock | Écran custom Flutter |
| **Choix&Cap — « Je décide »** | `DECISION` | Prise de décision | 🔴 **Déclaré, non implémenté** — aucune logique | — |
| **Gestion émotionnelle — « Je gère »** | *(à créer)* | Régulation émotionnelle | 🔴 **Non déclaré** — absent de `GameType`, carte visuelle inactive dans le hub | — |
